...
def test_generate_vtt_files(tmp_path):
    ...
    recognized = [
        ...
        TextedSegment(text="How are you", start=2.0, end=3.25, speaker="SPEAKER_01"),
    ]
    translated = [
        TranslatedTextedSegment(
        ...
        TranslatedTextedSegment(
            ...
            source_file=None, generated_file=None, speaker="SPEAKER_01",
        ),
    ]
    ...
    # Correctly formatted VTT timestamps (dot separator, not comma).
    ...
    # Speaker labels via <v SpeakerName> tags.
    assert "<v SPEAKER_00>Hello world" in source_text
    assert "<v SPEAKER_01>How are you" in source_text
    assert "<v SPEAKER_00>Privet mir" in translated_text
    assert "<v SPEAKER_01>Kak dela" in translated_text

    # Source VTT carries recognized text, translated VTT the translation.
    assert "Hello world" in source_text
    assert "Privet mir" in translated_text


def test_generate_vtt_no_speaker(tmp_path):
    """VTT generation works when speaker field is empty."""
    repo = LocalOnlyFileRepository("job2", str(tmp_path))

    recognized = [
        TextedSegment(text="Silent text", start=0.0, end=1.0, speaker=""),
    ]
    translated = [
        TranslatedTextedSegment(
            text="Silent text", start=0.0, end=1.0, translation="Tishi text",
            source_file=None, generated_file=None, speaker="",
        ),
    ]
    ...
    # No speaker tags when speaker is empty.
    assert "<v" not in source_text
    assert "<v" not in translated_text
    assert "Silent text" in source_text
    assert "Tishi text" in translated_text
